...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def _resolve_tenant(self, request: Request) -> TenantContext:
        ...
        if tenant_header:
            try:
                ...
                ctx = await self._lookup_tenant(tenant_id)
                if ctx:
                    return ctx
            except ValueError:
        ...
        # 3. Check JWT claim (handled by AuthMiddleware, stored in state)
        jwt_tenant_id = getattr(request.state, "jwt_tenant_id", None)
        if jwt_tenant_id:
            ctx = await self._lookup_tenant(jwt_tenant_id)
            if ctx:
                return ctx

        # 4. Default/fallback
        if self.default_tenant_id:
            return await self._lookup_tenant(
                uuid.UUID(self.default_tenant_id)
            ) or self._fallback_context()
    ...
    async def _lookup_tenant(self, tenant_id: uuid.UUID) -> Optional[TenantContext]:
        """Look up tenant by ID. In production, queries PostgreSQL + Redis cache."""
        cache_key = f"tenant:id:{tenant_id}"
        if cache_key in self._cache:
            data = self._cache[cache_key]
            return TenantContext(**data)
        # Placeholder: return a synthetic context for route wiring
        return TenantContext(
            tenant_id=tenant_id,
            name=f"Tenant-{tenant_id.hex[:8]}",
            slug=f"tenant-{tenant_id.hex[:8]}",
            subdomain=f"tenant-{tenant_id.hex[:8]}",
        )
