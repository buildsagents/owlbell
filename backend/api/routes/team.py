...
Invitations are managed in-memory (no dedicated DB table yet).
"""
...
# ---------------------------------------------------------------------------
# In-memory invitation store (no dedicated DB table exists)
# ---------------------------------------------------------------------------

_invites_db: dict[str, dict[str, Any]] = {}
...
async def invite_member(
    ...
    new_user = User(
        ...
        password_hash="",  # Will be set on invite acceptance
    )
    db.add(new_user)
    await db.flush()
...
async def send_invite(
    ...
    return SuccessResponse(
        data={
            ...
            "token": invite_token,
            "expires_at": _invites_db[invite_token]["expires_at"],
        },
        meta=ResponseMeta(request_id=""),
    )
...
async def accept_invite(
    ...
    invite = _invites_db.pop(body.token, None)
    if not invite:
