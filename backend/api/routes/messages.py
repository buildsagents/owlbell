...
async def get_message_stats(
    ...
    channel_query = await db.execute(
        select(Call.metadata_json["channel"].as_string(), func.count().label("count"))
        .where(
            Call.tenant_id == tenant.id,
            Call.voicemail_left == True,
            Call.metadata_json["channel"].isnot(None),
        )
        .group_by(Call.metadata_json["channel"])
    )
    ...
    priority_query = await db.execute(
        select(Call.metadata_json["priority"].as_string(), func.count().label("count"))
        .where(
            Call.tenant_id == tenant.id,
            Call.voicemail_left == True,
            Call.metadata_json["priority"].isnot(None),
        )
        .group_by(Call.metadata_json["priority"])
    )
    ...
    unread_count_result = await db.execute(
        select(func.count()).select_from(Call).where(
            Call.tenant_id == tenant.id,
            Call.voicemail_left == True,
            Call.metadata_json["read_by"].as_text().is_(None),
        )
    )
