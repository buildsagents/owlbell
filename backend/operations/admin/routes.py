...
class AdminService:
    ...
    async def submit_training_feedback(
        ...
        self._feedback_db.append(entry)
    ...
    async def get_feedback(
        ...
        feedback = self._feedback_db
        if call_id:
            feedback = [f for f in feedback if f["call_id"] == str(call_id)]
...
admin_router = APIRouter(dependencies=[Depends(RequireAdmin)])
