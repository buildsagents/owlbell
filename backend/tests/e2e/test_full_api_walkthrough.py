...
def _make_client(app) -> httpx.AsyncClient:
    """Create an httpx AsyncClient for testing a FastAPI app."""
...
# The project uses double-prefixed routes: API_PREFIX + router prefix
AUTH_PREFIX = f"{API_PREFIX}/auth"
CALLS_PREFIX = f"{CALLS_PREFIX}"
MESSAGES_PREFIX = f"{MESSAGES_PREFIX}"
APPOINTMENTS_PREFIX = f"{APPOINTMENTS_PREFIX}"
BUSINESS_PREFIX = f"{BUSINESS_PREFIX}"
INTEGRATIONS_PREFIX = f"{INTEGRATIONS_PREFIX}"
TEAM_PREFIX = f"{TEAM_PREFIX}"
...
class TestBusinessAPI:
    ...
    async def test_get_faq(self, client):
        ...
        list_resp = await client.get(f"{BUSINESS_PREFIX}/faq")
        faq_id = list_resp.json()["data"]["items"][0]["id"]
...
class TestAPIHeadersAndMiddleware:
    ...
    async def test_pagination_params(self, client):
        ...
        resp = await client.get(f"{CALLS_PREFIX}?page=1&per_page=5")
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
