...
def _verify_auth(request: Request) -> None:
    ...
    if not secret:
        raise HTTPException(status_code=503, detail="Agent tools secret not configured")
...
async def _resolve_tenant(payload: dict) -> tuple[Any, str]:
    """Resolve tenant from Retell agent_id (preferred) or to_number fallback."""
    call_obj = payload.get("call", {}) or {}
    agent_id = call_obj.get("agent_id") or payload.get("agent_id") or ""
