...
from api.dependencies import CurrentUser, RequireSuperAdmin, get_db_session
...
ONBOARDING_STEPS = [
    ...
    {"step": 8, "name": "First week review", "description": "7-day performance summary emailed by success specialist"},
]
...
async def _pipeline_steps_for_client(client_id: str) -> list[OnboardingStep]:
    """Map unified DB pipeline steps to agency OnboardingStep view."""
    from operations.onboarding import automation

    pipeline = await automation.get_pipeline(require_session_maker(), client_id)
    if pipeline is None:
        return [
            OnboardingStep(
                step=i + 1,
...
async def get_overview(user=CurrentUser) -> dict[str, Any]:
    ...
    for p in await automation.list_pipelines(require_session_maker()):
        label = "Complete" if p.get("is_complete") else f"{p.get('progress_percent', 0)}%"
...
async def get_client_detail(client_id: str, user=CurrentUser) -> dict[str, Any]:
    ...
    onboarding = await _pipeline_steps_for_client(str(tenant.id))
        ...
        try:
            ...
            if agent_result.get("success"):
                ...
                await upsert_for_tenant(
                    ...
                    retell_phone_number=assigned_phone,
...
async def advance_onboarding(client_id: str, user=CurrentUser) -> dict[str, Any]:
    """Complete the current step in the unified onboarding pipeline."""
    from backend.domain.onboarding.orchestrator import get_orchestrator
    from operations.onboarding import automation

    pipeline = await automation.get_pipeline(require_session_maker(), client_id)
    if pipeline is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Onboarding pipeline not found")

    current = pipeline.current_step
    if current is None:
        return {"success": True, "data": {"message": "Onboarding already complete", "complete": True}}

    orch = get_orchestrator()
    result = await orch.complete_step(
        client_id, current.step_id, notes="Advanced via agency dashboard",
    )

    if current.step_id == "go_live":
        async for db in get_db_session():
            tenant = await _get_tenant(db, client_id)
            tenant.status = "active"
            cfg = _cfg(tenant)
            cfg["onboarding_complete"] = result.get("is_complete", False)
            tenant.config_json = cfg
            await db.commit()
            break

    logger.info("agency.onboarding_advanced", client_id=client_id, step_id=current.step_id)
    return {
        "success": True,
        "data": {
            "client_id": client_id,
...
async def get_onboarding_pipeline(user=CurrentUser) -> dict[str, Any]:
    """Overview of all clients' onboarding status (unified DB pipeline)."""
    from operations.onboarding import automation
    from operations.onboarding.automation import DEFAULT_PIPELINE_STEPS

    pipelines = await automation.list_pipelines(require_session_maker())
    clients = []
    for p in pipelines:
        current = next(
            (s for s in p.get("steps", []) if s.get("status") not in ("completed", "skipped")),
            None,
        )
        clients.append({
            "client_id": p.get("tenant_id"),
            "name": p.get("tenant_name", ""),
            "progress_percent": p.get("progress_percent", 0),
            "total_steps": p.get("total_steps", 0),
            "complete": p.get("is_complete", False),
            "current_step_id": current.get("step_id") if current else None,
            "current_step_name": current.get("name") if current else "Complete",
        })
    ...
    return {
        ...
        "data": {
            ...
            "clients": sorted(clients, key=lambda x: x.get("progress_percent", 0), reverse=True),
        },
    }
