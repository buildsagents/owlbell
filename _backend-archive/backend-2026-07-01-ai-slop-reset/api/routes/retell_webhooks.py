...
from api.dependencies import RequireSuperAdmin, UserContext
...
from backend.db.tenant_integrations_service import resolve_tenant_by_retell_agent
...
async def _find_call_by_retell_id(db: Any, call_id: str) -> Optional[Call]:
    """Look up a call by indexed retell_call_id, with metadata_json fallback."""
    result = await db.execute(select(Call).where(Call.retell_call_id == call_id))
    call = result.scalar_one_or_none()
    if call:
        return call
    result = await db.execute(
        select(Call).where(Call.metadata_json["retell_call_id"].as_string() == call_id)
    )
...
async def _handle_call_started(payload: dict[str, Any]) -> None:
    ...
    async with _get_session() as db:
        ...
        if existing.scalar_one_or_none():
            logger.info("retell.call_already_recorded", call_id=call_id)
            return

        tenant = await resolve_tenant_by_retell_agent(db, agent_id)
        if not tenant and phone_number:
...
async def _handle_call_ended(payload: dict[str, Any]) -> None:
    ...
    async with _get_session() as db:
        call = await _find_call_by_retell_id(db, call_id)
...
@router.post("/test-token", include_in_schema=False)
async def create_test_web_call(payload: dict, _user: UserContext = RequireSuperAdmin):
