...
async def list_calls(
    ...
    recordings_by_call: dict[Any, list[Recording]] = {}
    if calls:
        call_ids = [c.id for c in calls]
        rec_result = await db.execute(
            select(Recording).where(
                Recording.call_id.in_(call_ids),
                Recording.is_deleted.is_(False),
            )
        )
        for rec in rec_result.scalars().all():
            recordings_by_call.setdefault(rec.call_id, []).append(rec)

    records = [
        _call_to_record(c, recordings=recordings_by_call.get(c.id))
        for c in calls
    ]
...
async def get_call_summary(
    ...
    total = (await db.execute(select(func.count()).select_from(base_subq))).scalar() or 0
    total_duration = (
        await db.execute(select(func.coalesce(func.sum(base_subq.c.duration_seconds), 0)))
    ).scalar() or 0
    ai_handled = (
        await db.execute(
            select(func.count()).select_from(base_subq).where(base_subq.c.ai_handled.is_(True))
        )
    ).scalar() or 0
    transferred = (
        await db.execute(
            ...
            .where(base_subq.c.transferred_to.isnot(None))
        )
    ).scalar() or 0
    missed = (
        await db.execute(
            select(func.count()).select_from(base_subq).where(base_subq.c.status == "no_answer")
        )
    ).scalar() or 0
