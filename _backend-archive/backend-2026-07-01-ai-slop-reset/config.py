...
class DatabaseSettings(BaseSettings):
    ...
    # Connection pool (direct Postgres)
    ...
    # PgBouncer / Supabase pooler — smaller client-side pool + no prepared statements
    use_pooler: Optional[bool] = Field(default=None, validation_alias="DB_USE_POOLER")
    pooler_pool_size: int = Field(default=5, validation_alias="DB_POOLER_POOL_SIZE")
    pooler_max_overflow: int = Field(default=2, validation_alias="DB_POOLER_MAX_OVERFLOW")
    ...
    @property
    def is_pooler_url(self) -> bool:
        """True when connecting through PgBouncer / transaction pooler."""
        if self.use_pooler is not None:
            return self.use_pooler
        url = self._raw_database_url().lower()
        if not url:
            return False
        if "pgbouncer=true" in url or ".pooler." in url or "pooler" in url:
            return True
    ...
    def effective_pool_size(self) -> int:
        return self.pooler_pool_size if self.is_pooler_url else self.pool_size

    @property
    def effective_max_overflow(self) -> int:
        return self.pooler_max_overflow if self.is_pooler_url else self.max_overflow
...
class FeatureFlagsSettings(BaseSettings):
    ...
    # FreeSWITCH ESL + orchestrator stack — off in production (Retell is call ingress)
    enable_legacy_freeswitch: bool = False
    # Dispatch background jobs to Celery worker service (fallback: in-process asyncio)
    enable_celery_workers: bool = Field(default=False, validation_alias="CELERY_WORKERS_ENABLED")
...
class IntegrationSettings(BaseSettings):
    ...
    gmail_from_email: str = "hello@owlbell.xyz"
    ...
    smtp_from_email: str = "hello@owlbell.xyz"
    ...
    stripe_price_setup_pro_plus: Optional[str] = None
