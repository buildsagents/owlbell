...
class TestSessionManager:
    ...
    def __init__(self, redis_client: FakeRedisClient) -> None:
        SessionManager = _session_manager_cls()
        self._delegate = SessionManager.__new__(SessionManager)
        self._redis = redis_client
        self._delegate._redis = redis_client
        self._delegate.event_bus = None
        self._delegate.redis_url = "fake://localhost"
...
class TestEventBus:
    ...
    def __init__(self, redis_client: FakeRedisClient) -> None:
        EventBus = _event_bus_cls()
        self._delegate = EventBus.__new__(EventBus)
        self._redis = redis_client
        self._delegate._redis = redis_client
        self._delegate._subscribers = set()
        self._delegate._listener_task = None
        self._delegate._running = False
        self._delegate._event_handlers = {}
        self._delegate.redis_url = "fake://localhost"
...
class EventCapture:
    ...
    def __init__(self, event_bus: TestEventBus) -> None:
        ...
        self._models = _orchestrator_models()
        self.events: List[Any] = []
        self._handler = None

    def start(self) -> None:
        """Start capturing events."""
        EventType = self._models["EventType"]
        for et in EventType:
...
class CallSimulator:
    ...
    async def simulate_inbound_call(
        ...
        ActiveSession = self._models["ActiveSession"]
        CallState = self._models["CallState"]
    ...
    async def end_call(self, call_id: str, reason: str = "hangup_by_caller"):
