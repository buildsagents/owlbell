...
class OnboardingIntakeRecord(Base):
    """Post-checkout concierge intake submitted from the marketing onboarding portal."""

    __tablename__ = "onboarding_intakes"

    id: Mapped[PgUUID] = mapped_column(
        PgUUID(as_uuid=True), primary_key=True, default=uuid4
    )
    tenant_id: Mapped[Optional[PgUUID]] = mapped_column(
        PgUUID(as_uuid=True),
    ...
    status: Mapped[str] = mapped_column(
        ...
        comment="submitted | linked | processed",
    )
    created_at: Mapped[datetime] = mapped_column(
        default=datetime.utcnow, nullable=False
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<OnboardingIntakeRecord(id={self.id}, email='{self.email}', "
