...
from backend.leads.agents.conversation import ConversationAgent
...
async def handle_replies() -> dict[str, Any]:
    ...
    for reply in replies:
        ...
        # Use Conversation Agent to classify + respond
        conv_agent = ConversationAgent()
        conv_result = await conv_agent.handle_reply(lead, body)

        classification = conv_result.get("classification", {}).get("classification", "neutral")
        response_data = conv_result.get("response", {})
        ...
        action = response_data.get("action", "send")
        reply_body = response_data.get("reply_body", "")

        if action == "flag_for_human" or action == "escalate":
            lead_store.mark_replied(lead["email"], f"flagged_{classification}")
            results.append({"email": lead["email"], "action": f"flagged_{classification}", "note": action})
            continue
