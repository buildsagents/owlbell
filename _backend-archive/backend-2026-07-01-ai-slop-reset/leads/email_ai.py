...
who run plumbing companies understand they're losing jobs to missed calls.
...
- Reference their specific plumbing business and location.
...
GENERATE_PROMPT = """Write a short cold email (max 150 words) for {business_name}, a plumbing company in {city}, {state}.
...
FOLLOWUP_1_PROMPT = """Write a follow-up email (max 120 words) for {business_name}, a plumbing company in {city}, {state}.

Context: We sent them a cold email {days_since} days ago about Owlbell (24/7 call answering for plumbing companies). They didn't reply.

The email should:
- Reference the previous email naturally (not "did you get my email")
- Use a different angle than "you're missing calls"
- Mention that other plumbing companies in {city} are already using it
...
FOLLOWUP_2_PROMPT = """Write a second follow-up email (max 100 words) for {business_name}, a plumbing company in {city}, {state}.

Context: They haven't replied to two previous emails about Owlbell (24/7 call answering for plumbing companies).

The email should:
- Be short and direct
- Lead with social proof ("Plumbing companies in {city} are adding $Xk/month with Owlbell")
...
FOLLOWUP_3_PROMPT = """Write a final follow-up email (max 80 words) for {business_name}, a plumbing company in {city}, {state}.
