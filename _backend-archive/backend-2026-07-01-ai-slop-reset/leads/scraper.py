...
plumbing companies by city. Returns structured lead data.
...
async def text_search(trade: str, city: str, state: str, max_results: int = 20) -> list[dict[str, Any]]:
    """Search Google Places for plumbing companies by trade + location.
...
async def get_place_details(place_id: str) -> Optional[dict[str, Any]]:
    ...
    params = {
        "place_id": place_id,
        "key": key,
        "fields": "name,formatted_phone_number,international_phone_number,website,formatted_address,rating,user_ratings_total,business_status,opening_hours,reviews",
    }
...
def parse_contractor(place: dict[str, Any], details: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    # Keep only the review fields the Signal Agent needs — review *text* is the
    # single strongest buying signal (mentions of missed calls / voicemail).
    reviews = [
        {
            "text": r.get("text", ""),
            "rating": r.get("rating"),
...
async def find_contractors(
    ...
    """Find plumbing companies across cities.

    Args:
        trades: List of trade keys. Defaults to ["plumbing"].
