...
# ── Agent Pipeline: Intel → Opportunity → Personalisation → Outreach ──


async def _run_agent_pipeline(lead: dict[str, Any]) -> dict[str, Any]:
    """Run the full agent pipeline on a lead if it doesn't have enrichment yet."""
    if lead.get("_outreach") and lead.get("_intel_status") == "complete":
        return lead

    results = await run_intel_pipeline([lead], max_leads=1)
    if results:
        return results[0]
    return lead


async def _get_outreach_variant(lead: dict[str, Any]) -> tuple[str, str]:
    """Get the best email variant from outreach agent data."""
    outreach = lead.get("_outreach", {})
    variants = outreach.get("variants", [])
    recommended = outreach.get("recommended_variant", 0)

    if variants:
        best = variants[recommended] if recommended < len(variants) else variants[0]
        subject = best.get("subject_line", "")
        body = best.get("body", "")
        if body:
            return subject, body

    # Fallback to stored AI body
    ai_body = lead.get("_ai_body", "")
    ai_subject = lead.get("_ai_subject", "")
    if ai_body:
        return ai_subject or f"Quick question about {lead.get('name', 'your business')}", ai_body
...
async def send_initial(
    ...
    """Send first email to a lead using agent-generated outreach variants."""
    ...
    # Run agent pipeline if not already enriched
    lead = await _run_agent_pipeline(lead)
    subject, body = await _get_outreach_variant(lead)

    if not body:
        # Final fallback — direct AI generation
        from backend.leads.email_ai import generate_email
...
async def run_initial_batch(
    ...
    # Run agent pipeline on leads needing enrichment
    needs_intel = lead_store.get_leads_needing_intel(limit=max_per_batch)
    if needs_intel:
        logger.info("outreach.running_agent_pipeline", count=len(needs_intel))
        await run_intel_pipeline(needs_intel, max_leads=max_per_batch)

    # Score unscored leads first
    from backend.leads.lead_scorer import score_pending_leads
    await score_pending_leads(limit=50)

    # Get leads ready for outreach (agent-processed + qualified + with email)
    pending = lead_store.get_leads_ready_for_outreach(limit=max_per_batch)
    if not pending:
