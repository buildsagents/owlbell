...
def get_leads_needing_intel(limit: int = 30) -> list[dict[str, Any]]:
    """Leads without company intelligence enrichment."""
    data = _load()
    pending = [
        l for l in data["leads"]
        if l.get("status") == "new"
        and not l.get("_intel_status")
        and not l.get("_intelligence")
        and l.get("website")
    ]
    return pending[:limit]


def get_leads_qualified(limit: int = 30) -> list[dict[str, Any]]:
    """Leads that passed opportunity qualification with intel."""
    data = _load()
    qualified = [
        l for l in data["leads"]
        if l.get("status") == "new"
        and l.get("_intel_status") == "complete"
        and l.get("_opportunity_qualified") is True
    ]
    qualified.sort(key=lambda l: l.get("_opportunity_confidence", 0) or 0, reverse=True)
    return qualified[:limit]


def get_leads_ready_for_outreach(limit: int = 20) -> list[dict[str, Any]]:
    """Leads fully processed by all agents, ready for sending."""
    data = _load()
    ready = [
        l for l in data["leads"]
        if l.get("status") == "new"
    ...
    ready.sort(key=lambda l: l.get("_opportunity_confidence", 0) or 0, reverse=True)
    return ready[:limit]
...
def stats() -> dict[str, Any]:
    ...
    onboarded = sum(1 for l in leads if l.get("status") == "onboarded")
    ...
    intel_complete = sum(1 for l in leads if l.get("_intel_status") == "complete")
    qualified_opp = sum(1 for l in leads if l.get("_opportunity_qualified") is True)
    ready_for_outreach = sum(1 for l in leads if l.get("_outreach"))
    needs_intel = len(get_leads_needing_intel(limit=9999))
    ...
    return {
        ...
        "agent_pipeline": {
            ...
            "qualified_opportunity": qualified_opp,
