...
async def run_initial_pipeline(
    ...
    outreach_results = await run_initial_batch(max_per_batch=max_daily_outreach, dry_run=dry_run)
    ...
    if output_file:
        ...
        out_path.write_text(json.dumps({
            ...
            "sent": outreach_results.get("sent", []),
            "skipped": outreach_results.get("skip", []),
...
async def run_followup_pipeline(
    ...
    results = await run_followup_batch(max_per_batch=max_daily_outreach, dry_run=dry_run)
    ...
    return {
        ...
        "mode": "followup",
        "elapsed_s": round(elapsed, 1),
        "followups_sent": results.get("sent", 0),
    }
