...
from backend.dependencies import close_all_dependencies
from backend.di import get_container, set_container
from backend.di.container import DependencyContainer
...
# Auth-exempt paths (single source of truth — api/main.py re-exports these)
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    # -- 0–2. DI container startup (database + redis) ----------------------
    container: DependencyContainer = app.state.container

    # -- 1–2. Database + Redis (via container) -----------------------------
    try:
        await container.startup(settings)
        logger.info("app.startup.database.ok")
        app.state.redis = await container.redis()
        logger.info("app.startup.redis.ok")
    except Exception as exc:
        startup_errors.append(f"container: {exc}")
        logger.error("app.startup.container.failed", error=str(exc))

    # -- 3. AI pipeline (container may have warmed it during startup) -------
    if (
        settings.env != "testing"
        and (settings.features.enable_ai_greeting or settings.features.enable_call_transcription)
    ):
        try:
            ai_pipeline = await container.ai_pipeline()
    ...
    # -- 4–6. Legacy FreeSWITCH / orchestrator (disabled in production) -------
    app.state.fs_healthy = False
    if settings.features.enable_legacy_freeswitch and not settings.is_testing:
        try:
        ...
        logger.info("app.startup.legacy_freeswitch.skipped")

    # -- 7. AI model verification -------------------------------------------
    if settings.is_testing:
        app.state.ai_healthy = False
        logger.info("app.startup.ai_models.skipped_testing")
    else:
        try:
    ...
    # Close DI container and shared dependencies
    try:
...
def _register_api_routers(app: FastAPI, settings: Settings) -> None:
    ...
    from api.routes import retell_webhooks, agent_tools
    ...
    from operations.onboarding.routes import router as ops_onboarding_router

    # NOTE: each router self-prefixes (e.g. APIRouter(prefix="/auth")).
    ...
    app.include_router(ops_onboarding_router, prefix=f"{API_PREFIX}/ops")
    logger.info("app.routers.api.registered")

    # Legacy orchestrator gateway (FreeSWITCH path only)
    if settings.features.enable_legacy_freeswitch:
        try:
        ...
        logger.info("app.routers.orchestrator.skipped_legacy_disabled")
...
def create_app(env: Optional[str] = None, settings: Optional[Settings] = None) -> FastAPI:
    ...
    # Attach settings and DI container to app state
    ...
    app.add_middleware(
        ...
        allow_origins=(
            ...
            else ["https://*.owlbell.xyz", "https://owlbell.xyz"]
        ),
