...
describe("PRICING_PLANS", () => {
  ...
  it("aligns setup fees with backend MANAGED_PLANS", () => {
    expect(PRICING_PLANS.find((p) => p.id === "pro")?.setupFee).toBe(4000);
    expect(PRICING_PLANS.find((p) => p.id === "pro_plus")?.setupFee).toBe(8000);
  });
});
...
describe("formatMonthlyPrice", () => {
  it("formats Growth rate for UI", () => {
    expect(formatMonthlyPrice(getCheckoutDisplay("pro").monthly)).toBe("£3,997/mo");
  });

  it("formats Launch rate for UI", () => {
    expect(formatMonthlyPrice(getCheckoutDisplay("basic").monthly)).toBe("£1,197/mo");
  });
});

describe("formatSetupFee", () => {
  it("formats setup fee copy", () => {
    expect(formatSetupFee(4000)).toBe("£4,000 one-time setup");
  });
});
